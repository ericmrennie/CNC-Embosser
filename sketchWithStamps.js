...
function send() {
  // console.log('hello');
  let strokesIncrementor = 0;
  console.log('actions', actions);
  console.log('strokes', strokes);
  console.log('placedStamps', placedStamps);
  for (let i=0; i<actions.length; i++) {
    if (actions[i].type === 'stroke') {
      strokesIncrementor += strokes[actions[i].index].segments.length;
    } else if (actions[i].type === 'stamp') {
      console.log("stamps: ", placedStamps.length);
    }
  }
}